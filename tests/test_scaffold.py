...
from apb_studio.scaffold import build_corpus, detect_level, discover, find_input, read_headers
...
def _dataset(root, module, ds, header_cols, *, input_name="input_file.tsv", param_name="param_0..txt"):
    d = root / module / ds
...
def test_build_corpus_clean_module_names_vendor_per_dataset(tmp_path):
    ...
    _dataset(root, "Results_quant_ion_DIA_AIF", "h_peaks", ["Peptide", "Quality"], input_name="input_file.txt")
    _dataset(root, "Results_quant_peptidoform_DDA", "h_pep", ["Peptide"], param_name="parameters.txt")
    ...
    assert set(corpus["modules"]) == {"Results_quant_ion_DIA_AIF", "Results_quant_peptidoform_DDA"}

    aif = {d["vendor"]: d for d in corpus["modules"]["Results_quant_ion_DIA_AIF"]["datasets"]}
...
def test_build_corpus_output_is_pipeline_valid(tmp_path):
    ...
    _dataset(root, "Results_quant_ion_DDA", "h_peaks", ["Peptide"], input_name="input_file.txt")
...
def test_build_corpus_level_is_vendor_native_not_module_name(tmp_path):
    ...
    root = tmp_path / "data"
    _dataset(root, "Results_quant_ion_DDA", "h_wombat", ["WombatCol"], input_name="input_file.csv")
    _dataset(root, "Results_quant_ion_DDA", "h_mq", ["MqCol"], input_name="input_file.txt")

    def rec(headers):
        return {"WombatCol": "wombat", "MqCol": "maxquant"}.get(headers[0])

    corpus, _ = build_corpus(root, recognizer=rec)
    by_vendor = {d["vendor"]: d for d in corpus["modules"]["Results_quant_ion_DDA"]["datasets"]}
    assert by_vendor["wombat"]["level"] == "peptidoform"  # vendor-native, not the module's "ion"
