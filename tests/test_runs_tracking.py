"""Converted-runs tracking + result summaries (relocated from apb's test_ui_support_converted_runs).

Exercises apb_studio.support against tmp_path run dirs; conversion metadata now comes from the
``command.json`` sidecar that subprocess_adapter writes (not a parsed CLI argv).
"""
...
def test_list_converted_runs_joins_catalog_metadata(tmp_path, monkeypatch) -> None:
    ...
    # subprocess_adapter records the conversion in command.json (not the console.log argv).
...
def test_summarize_shows_full_search_parameters() -> None:
    ...
    assert params["precursor_mass_tolerance"] == {"value": 15.0, "unit": "ppm", "mode": "absolute"}
    assert params["fragment_mass_tolerance"] == {"value": 20.0, "unit": "ppm", "mode": "absolute"}
