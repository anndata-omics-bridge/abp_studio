...
Browse the ProteoBench test corpus (via ``apb_studio.support``), filter by software / size /
conversion target, and convert a selected dataset to an AnnData level or a MuData. The conversion
runs as a **background subprocess** that shells out to the ``apb convert`` CLI
(``apb_studio.conversion.subprocess_adapter`` + ``runner``), streaming to a log file; the UI polls
with mo.ui.refresh and shows a live status panel — so heavy fragment/MuData runs don't freeze the
app and the log/result open in a browser.

Run via the apb_studio Makefile (`make test-tool`) or directly:
    marimo run src/apb_studio/ui/test_tool.py
Requires the ``apb`` CLI on PATH (``uv pip install -e ../anndata_proteomics_bridge``).
"""
...
def _():
    ...
    from anndata_proteomics.converters.pipeline import LEVELS, MUDATA

    from apb_studio import support as ui
    from apb_studio.conversion import runner
    from apb_studio.conversion import subprocess_adapter as adapter
    from apb_studio.ui import panels

    return LEVELS, MUDATA, adapter, datetime, mo, panels, runner, ui
...
def _(mo):
    mo.md(
        ...
        the background (shelling out to `apb convert`); the panel on the right shows live status +
        console log, with the result and log openable in the browser.
        """
    )
    return
...
@app.cell
def _(LEVELS, MUDATA, catalog, mo):
    targets = LEVELS + [MUDATA]
...
def _(mo):
    ...
    #   get_job   – the runner.Job handle for the current/last conversion
...
def _(
    ...
    if convert_button.value and _row is not None and not running:
        ...
        job = adapter.start_conversion(
            ui.dataset_path(_row["input_file_path"]),
            slug=_row["slug"],
            target=_target,
            params=_row["param_path"],
            outdir=_outdir,
            runner=runner,
            input_rel=_row["input_file_path"],
            run_key=_run_key,
        )
        set_job(job)
        running = True
    ...
    if job is None:
        ...
        # argv form: ["apb", "convert", <data>, (<level>?), "--params", ..., "--software", <slug>, ...]
        _cmd = list(job.command)
        _job_slug = _cmd[_cmd.index("--software") + 1] if "--software" in _cmd else ""
        _job_target = _cmd[3] if len(_cmd) > 3 and not _cmd[3].startswith("--") else MUDATA
